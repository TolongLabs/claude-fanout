...
Dispatch headless Claude Code runs as background workers, pointed at CLIProxyAPI so they spend a few cents of OpenRouter
...
**2. List the OpenRouter models the proxy serves.** Only the `openrouter` block of `config.yaml` counts. The proxy also
serves whatever OAuth logins it holds, and those are not worker models: they are not cheap, and a dead login fails every
call.
...
**3. Ask the user which model to use, unless they already named one.** Use `AskUserQuestion`, one question, with the
default first and marked, offering only what step 2 listed. A model the user named earlier in the same session is a
standing answer: state it back and dispatch.
...
**Adding a cheap worker model.** Every model below supports tool calls and a large context, and costs under a dollar per
million output tokens. Prices are OpenRouter's on 2026-09-07; re-check before relying on one.
...
`acceptEdits` refuses every Bash command, measured: a worker's three `awk` line-width checks were all denied. Add
`--allowedTools "Bash(bun test:*)"` for the commands a brief asks the worker to run, and nothing wider.
...
The JSON result reports `total_cost_usd` as if Anthropic served the model. **It is fictional.** Real cost is on the
proxy's upstream. Never quote it.
...
part - they tell the user whether the next fan-out should use a stronger model or a tighter brief.
