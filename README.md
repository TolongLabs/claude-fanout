...
**Fan work out to headless Claude Code workers on a cheap proxy model, up to six at once.**
...
- **Headless Claude Code Workers on a Cheap Proxy Model.** Each worker is the full Claude Code agent in one directory,
  pointed at CLIProxyAPI so it spends a few cents of OpenRouter credit instead of the Claude plan.
...
1. **Check the Prerequisites.** You need Claude Code, CLIProxyAPI installed and configured with at least one OpenRouter
   model, and `curl` and `jq`.

1. **Clone Into the Global Skills Directory**, then restart Claude Code so the skill loads. It is available as
   `/claude-fanout` from then on.

   ```bash
   git clone https://github.com/TolongLabs/claude-fanout ~/.claude/skills/claude-fanout
   ```

1. **Bring the Proxy Up and Choose a Model.** Start the proxy if its port is closed, then list the OpenRouter models it
   serves. Only the `openrouter` block of its config counts. Never print the token, which is a local secret and leaks
   into transcripts.
...
1. **Verify.** The `total_cost_usd` in the JSON result is fictional, because the proxy served the model, so never quote
   it. Read the report, then run the tests yourself, because a green run from a worker proves the worker's tests agree
   with the worker's code and nothing else:
...
2026-09-07, so re-check before relying on one. `SKILL.md` shows the three lines that add one to the proxy.
...
- **The Cost Figure Is Fictional.** The JSON result reports `total_cost_usd` as if Anthropic served the model, and the
  real cost is on the proxy's upstream. Never quote it.
